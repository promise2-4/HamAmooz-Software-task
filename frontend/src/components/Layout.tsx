...
export function Layout() {
  ...
  const namespaceMatch = location.pathname.match(/^\/clusters\/(\d+)\/namespaces\/(\d+)/);
      ...
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="Main navigation">
          ...
          <div className="nav-tree" aria-label="Infrastructure sections">
            <Link className={isRoot ? "tree-link active" : "tree-link"} to="/clusters"><ClusterIcon /><span>Clusters</span></Link>
            {clusterId && <Link className={clusterMatch && !namespaceId ? "tree-link active" : "tree-link"} to={`/clusters/${clusterId}/namespaces`}><LayersIcon /><span>Namespaces</span></Link>}
            {clusterId && namespaceId && <Link className={namespaceMatch && !appMatch ? "tree-link active" : "tree-link"} to={`/clusters/${clusterId}/namespaces/${namespaceId}`}><BoxIcon /><span>Applications</span></Link>}
            {appMatch && !namespaceId && <span className="tree-link current"><BoxIcon /><span>Applications</span></span>}
            {appMatch && <span className="tree-link active current" aria-current="page"><DetailIcon /><span>App details</span></span>}
          </div>
        </nav>
