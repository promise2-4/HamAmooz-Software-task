...
export function AppsPage() {
  ...
  return <div className="page"><nav className="breadcrumbs"><Link to="/clusters">Clusters</Link><i>/</i><Link to={`/clusters/${clusterId}/namespaces`}>{cluster?.name ?? "Cluster"}</Link><i>/</i><strong>{namespace?.name ?? "Namespace"}</strong></nav><header className="page-header"><div><span className="eyebrow">Namespace workspace</span><h1>{namespace?.name ?? "Applications"}</h1><p>Deploy and inspect the workloads running in this namespace.</p></div><div className="header-actions"><button className="button secondary" onClick={() => void load()}><RefreshIcon /> Refresh</button>{isAdmin && <button className="button primary" onClick={() => setShowCreate(true)}><PlusIcon /> Deploy app</button>}</div></header>{loading ? <PageLoader label="Loading applications" /> : error ? <ErrorState message={error} onRetry={() => void load()} /> : apps.length === 0 ? <EmptyState icon={<BoxIcon />} title="No applications deployed" description={isAdmin ? "Deploy a container image to create the first workload." : "There are no applications available to view."} action={isAdmin ? <button className="button primary" onClick={() => setShowCreate(true)}><PlusIcon /> Deploy app</button> : undefined} /> : <section className="resource-grid app-grid">{apps.map((app) => <article className="resource-card app-card" key={app.id}><div className="resource-card-top"><span className="resource-icon"><BoxIcon /></span><StatusBadge status={appStatus(app)} /></div><div><h2>{app.name}</h2><p className="mono truncate">{app.image}</p></div><div className="mini-metrics"><div><span>Ready pods</span><strong>{app.ready_replicas ?? 0} / {app.replicas}</strong></div><div><span>CPU</span><strong>{app.cpu_request || "Not set"}</strong></div><div><span>Memory</span><strong>{app.memory_request || "Not set"}</strong></div></div><div className="card-footer">{isAdmin && <button className="icon-button danger-ghost" onClick={() => setDeleting(app)} aria-label={`Delete ${app.name}`}><TrashIcon /></button>}<Link className="card-link" to={`/apps/${app.id}?cluster=${clusterId}`}>View details <ArrowIcon /></Link></div></article>)}</section>}{isAdmin && showCreate && <Modal title="Deploy application" eyebrow={`Namespace: ${namespace?.name ?? namespaceId}`} onClose={() => setShowCreate(false)}><AppForm submitLabel="Deploy application" busy={busy} error={formError} onSubmit={createApp} onCancel={() => setShowCreate(false)} /></Modal>}{isAdmin && deleting && <ConfirmDialog title={`Delete ${deleting.name}?`} description="The Kubernetes Deployment and its backend record will be removed. This action cannot be undone." busy={busy} onConfirm={deleteApp} onClose={() => setDeleting(undefined)} />}</div>;
}
